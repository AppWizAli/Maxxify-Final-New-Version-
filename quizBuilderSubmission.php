...
// Fetch all answers for this quiz with MCQ details
$sql = "
  SELECT 
    qa.question_index,
    qa.selected_answer,
...
foreach ($answers as $ans) {
    if (strtolower($ans['selected_answer']) === strtolower($ans['correct_option'])) {
        $correct++;
    } else {
        $incorrect++;
    }
...
// Group answers by topic for breakdown
$topicStats = [];
foreach ($answers as $ans) {
    $tid = $ans['topic_id'] ?? null;
    
    if ($tid !== null) {
        $key = 'topic_' . $tid;
        $name = $ans['topic_name'] ?? 'Unknown Topic';
    } else {
        $key = 'unknown';
        $name = 'Unknown Topic';
    }
...
// Get display info
$displayName = htmlspecialchars($quiz['deck_name']);
$displayType = 'Custom Quiz';
?>
...
<!-- ✅ Right Content Section -->
  ...
  <button
    onclick="window.location.href='quizBuilderAnswers.php?quiz_id=<?= $quizId ?>'"
...
<div class="max-w-[900px] mx-auto bg-white rounded-lg shadow-md p-4 overflow-x-auto">
  ...
  <table class="w-full text-left border-collapse font-[Manrope] text-[15px] leading-tight">
    ...
    <tbody>
<?php foreach ($topicStats as $key => $stat): ?>
<tr class="border-b hover:bg-gray-50">
  <td class="py-2 px-2">🧬</td>
  <td class="py-2 px-2"><?= htmlspecialchars($stat['name']) ?></td>
  <td class="py-2 px-2"><?= $stat['total'] ?></td>
  <td class="py-2 px-2"><?= $stat['correct'] ?></td>
</tr>
<?php endforeach; ?>
</tbody>
  </table>
